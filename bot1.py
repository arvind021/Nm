...
def main_menu_keyboard():
    ...
    return [
        ...
        [Button.text("📋 List Accounts"), Button.text("📥 Export Session")],
        [Button.text("👤 Report User"), Button.text("🤖 Report Bot")],
        [Button.text("👥 Report Group"), Button.text("📡 Report Channel")],
        [Button.text("🔥 Batch Report"), Button.text("📊 Statistics")],
        [Button.text("🌐 Proxies"), Button.text("❌ Logout")]
    ]
...
def proxy_keyboard():
    ...
    keyboard = [[Button.text(p)] for p in proxies]
    keyboard.append([Button.text("none")])
    keyboard.append([Button.text("❌ Back")])
    return keyboard
...
async def export_session_button(event):
    ...
    keyboard = [[Button.text(acc[0])] for acc in accounts]
    keyboard.append([Button.text("❌ Back")])
...
⚠️ Keep this secret!
"""
            await event.reply(export_msg, buttons=main_menu_keyboard())
        else:
            await event.reply(msg, buttons=main_menu_keyboard())
        del bot.user_states[user_id]
...
if __name__ == "__main__":
    TelegramClient(
        StringSession(bot.config.get('BOT_TOKEN')),
        int(bot.config['API_ID']),
        bot.config['API_HASH']
    ).start(bot_token=bot.config.get('BOT_TOKEN'))
