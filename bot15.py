...
class UltraBot:
    ...
    async def get_client(self, account_name):
        ...
        try:
            ...
            # ✅ 30 second timeout - slow connections ke liye
            await asyncio.wait_for(client.connect(), timeout=60)
            authorized = await asyncio.wait_for(client.is_user_authorized(), timeout=40)
            if authorized:
                self.active_clients[account_name] = client
                return client
            await client.disconnect()
        except asyncio.TimeoutError:
            log.warning(f"get_client({account_name}): connection timeout")
    ...
    async def add_account_from_session(self, name, session_str, phone=None, proxy_name=None):
        try:
            ...
            # ✅ 30 second timeout lagao
            try:
                await asyncio.wait_for(client.connect(), timeout=60)
            except asyncio.TimeoutError:
                return False, "❌ Connection timeout — dobara try karo"

            try:
                authorized = await asyncio.wait_for(client.is_user_authorized(), timeout=40)
            except asyncio.TimeoutError:
                await client.disconnect()
                return False, "❌ Auth check timeout — dobara try karo"
            ...
            try:
                ...
                await client.disconnect()
                return False, "❌ Account info timeout — dobara try karo"
    ...
    def _is_pyrogram_session(self, session_str):
        ...
        # Telethon session '1AgAB...' se shuru hoti hai
        if s.startswith('1AgAB') or s.startswith('1AgAC'):
            return False

        # Pyrogram v2 session ki length ~356 chars hoti hai (base64url)
        # Aur sirf alphanumeric + _ + - hoti hai
        import re
        if re.match(r'^[A-Za-z0-9_-]{100,400}$', s):
            # Try decode karke verify karo
            try:
                import base64
                padded = s + '=' * (-len(s) % 4)
                data   = base64.urlsafe_b64decode(padded)
                # Pyrogram v2: 1(dc) + 256(auth_key) + 8(user_id) + 1(is_bot) = 266 bytes
                if len(data) >= 266:
                    dc_id = data[0]
                    if 1 <= dc_id <= 5:  # Valid DC 1-5
                        return True
            except:
                pass
    ...
    async def _pyrogram_to_telethon(self, pyrogram_session):
        ...
        try:
            ...
            if len(data) < 266:
                log.error(f"Pyrogram data too short: {len(data)}")
                return None

            # Pyrogram v2 format:
            # [0]      = dc_id (1 byte)
            # [1:257]  = auth_key (256 bytes)
            # [257:265] = user_id (8 bytes, big-endian)
            # [265]    = is_bot (1 byte)
            dc_id    = data[0]
            auth_key = data[1:257]
            user_id  = struct.unpack('>q', data[257:265])[0]

            log.info(f"Pyrogram convert: DC{dc_id}, user_id={user_id}")
            ...
            sess = SS()
            sess.set_dc(dc_id, self._get_dc_ip(dc_id), 443)
            sess.auth_key = AuthKey(data=auth_key)

            result = sess.save()
            log.info(f"Pyrogram converted successfully, length={len(result)}")
            return result
...
MENU_BUTTONS = {
    '🔐 Login', '❌ Logout', '❌ Back',
    '➕ Add Account', '📤 Import Session', '📥 Export Session',
    '📋 Accounts', '📊 Statistics',
    '🗑️ Delete Account', '🏥 Health Check',
    '🎯 Report', '💥 Mass Report',
    ...
    '🌐 IP Checker',
    '🔥 Batch Targets', '⏰ Scheduler',
    '💾 Saved Targets', '🔍 Target Info',
    '📤 Export CSV', '⚙️ Settings',
    # Sub-menu buttons
    '💾 Save New Target', '🚀 Report Saved Target', '🗑️ Delete Saved Target',
    '➕ New Schedule', '🗑️ Delete Schedule',
...
async def h_live_counter(event):
    ...
    rate_str = f"{(a_ok/a_total*100):.1f}%" if a_total else "N/A"
    text = (
