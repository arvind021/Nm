...
class UltraBot:
    ...
    async def get_client(self, account_name):
        ...
        try:
            ...
            # ✅ 15 second timeout - hang nahi hoga
    ...
    async def add_account_from_session(self, name, session_str, phone=None, proxy_name=None):
        try:
            ...
            # ✅ 15 second timeout lagao
            try:
                await asyncio.wait_for(client.connect(), timeout=15)
            except asyncio.TimeoutError:
                return False, "❌ Connection timeout — VPS ka internet check karo"

            try:
                authorized = await asyncio.wait_for(client.is_user_authorized(), timeout=10)
            except asyncio.TimeoutError:
                await client.disconnect()
                return False, "❌ Auth check timeout — session expire ho gaya hoga"
            ...
            try:
                ...
                await client.disconnect()
                return False, "❌ Account info timeout"
